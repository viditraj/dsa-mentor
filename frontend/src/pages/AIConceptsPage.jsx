...
import {
  Cpu, ChevronDown, ChevronRight, BookOpen, CheckCircle2,
  Clock, Building2, Zap, ArrowRight, Brain, Sparkles, ExternalLink,
  Play, ChevronLeft, Lightbulb, Tag, Search, Filter, Youtube,
  AlertTriangle, MessageSquare, Code2, Globe, BarChart3, Layers, Binary,
  Loader2, Star, X
} from 'lucide-react'
import {
  getAIConceptsOverview, getAIConcept,
...
const ProgressBar = ({ value, max, className = '' }) => {
  ...
  return (
    <div className={`w-full bg-gray-800 rounded-full h-2 ${className}`}>
      <div
        className="h-2 rounded-full transition-all duration-500 bg-gradient-to-r from-violet-500 to-fuchsia-500"
        style={{ width: `${pct}%` }}
      />
    </div>
  )
    ...
    components={{
      code({ inline, className, children, ...props }) {
        ...
        return !inline && match ? (
          ...
          <code className="bg-gray-800 px-1.5 py-0.5 rounded text-sm text-violet-300" {...props}>{children}</code>
        )
      },
      h1: ({ children }) => <h1 className="text-2xl font-bold mt-6 mb-3">{children}</h1>,
      h2: ({ children }) => <h2 className="text-xl font-bold mt-5 mb-2 text-violet-300">{children}</h2>,
...
/* ─── Phase icons ─── */
const phaseIcons = { 1: Brain, 2: BarChart3, 3: Layers, 4: Cpu }
const phaseColors = {
  1: 'from-violet-500 to-fuchsia-500',
  2: 'from-fuchsia-500 to-pink-500',
  3: 'from-pink-500 to-rose-500',
  4: 'from-rose-500 to-orange-500',
}
...
export default function AIConceptsPage({ userId, user }) {
  ...
  const fetchData = useCallback(async () => {
    try {
      const data = await getAIConceptsOverview()
      setOverview(data)
    } catch (e) {
      console.error('Failed to load AI/ML data:', e)
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  const loadConcept = async (conceptId) => {
    ...
    try {
      const data = await getAIConcept(conceptId)
    ...
    return (
      <div className="flex items-center justify-center h-96">
        <div className="text-center">
          <Cpu className="w-12 h-12 mx-auto mb-4 text-violet-400 animate-pulse" />
          <p className="text-gray-400">Loading AI/ML curriculum...</p>
        </div>
      </div>
    )
  }
  ...
  if (!overview) {
    return (
      <div className="text-center py-20">
        ...
        <button onClick={fetchData} className="mt-4 px-4 py-2 bg-violet-500/20 text-violet-400 rounded-lg hover:bg-violet-500/30 transition-colors">
          Retry
        </button>
      </div>
    )
  ...
  if (view === 'concept-detail') {
    ...
    return (
      <div className="animate-fade-in">
        {/* Back button */}
        <button onClick={goBack} className="flex items-center gap-2 text-gray-400 hover:text-violet-400 transition-colors mb-6">
        ...
        <div className="glass-card p-6 mb-6">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {(conceptDetail.tags || []).map(tag => (
              <span key={tag} className="text-xs px-2 py-1 rounded-full bg-violet-500/10 text-violet-400 border border-violet-500/20">
          ...
          {/* Math Notation */}
          {conceptDetail.math_notation && (
            <div className="mt-3 p-3 bg-fuchsia-500/10 border border-fuchsia-500/20 rounded-lg">
              <div className="flex items-center gap-2 text-fuchsia-400 text-sm font-semibold mb-1">
                <Binary className="w-4 h-4" /> Key Formula
              </div>
              <code className="text-sm text-fuchsia-300 font-mono">{conceptDetail.math_notation}</code>
            </div>
          )}
        </div>
        ...
        <div className="glass-card p-6">
            ...
            <div className="animate-fade-in">
              ...
              {conceptDetail.key_points?.length > 0 && (
                <div className="mt-6 p-4 bg-gray-800/50 rounded-lg border border-gray-700/50">
                  <h3 className="text-lg font-semibold mb-3 flex items-center gap-2">
                    <CheckCircle2 className="w-5 h-5 text-violet-400" /> Key Points
                  </h3>
              ...
              {conceptDetail.real_world_examples?.length > 0 && (
                <div className="mt-6 p-4 bg-fuchsia-500/10 rounded-lg border border-fuchsia-500/20">
                  <h3 className="text-lg font-semibold mb-3 flex items-center gap-2">
                    <Globe className="w-5 h-5 text-fuchsia-400" /> Real-World Applications
                  </h3>
                  <ul className="space-y-2">
                    {conceptDetail.real_world_examples.map((ex, i) => (
                      <li key={i} className="flex items-start gap-2 text-gray-300 text-sm">
                        <Building2 className="w-4 h-4 text-fuchsia-400 mt-0.5 flex-shrink-0" />
                        {ex}
                      </li>
                    ))}
                  </ul>
              ...
              {conceptDetail.diagram_description && (
                <div className="mt-6 p-4 bg-pink-500/10 rounded-lg border border-pink-500/20">
                  <h3 className="text-lg font-semibold mb-3 flex items-center gap-2">
                    <Layers className="w-5 h-5 text-pink-400" /> Visual Overview
                  </h3>
          ...
          {activeTab === 'code' && (
            <div className="animate-fade-in">
              <h3 className="text-lg font-semibold mb-3 flex items-center gap-2">
                <Code2 className="w-5 h-5 text-violet-400" /> Code Example
              </h3>
              {conceptDetail.code_example ? (
                <Md>{`\`\`\`python\n${conceptDetail.code_example}\n\`\`\``}</Md>
              ) : (
                <p className="text-gray-500">No code example available yet.</p>
              )}
            </div>
          )}
          ...
          {activeTab === 'interview' && (
            <div className="animate-fade-in space-y-6">
              ...
              {conceptDetail.related_concepts?.length > 0 && (
                <div>
                  <h3 className="text-lg font-semibold mb-3 flex items-center gap-2">
                    <ArrowRight className="w-5 h-5 text-violet-400" /> Related Concepts
                  </h3>
                  <div className="flex flex-wrap gap-2">
                    {conceptDetail.related_concepts.map(id => (
                      <button
                        ...
                        className="px-3 py-1.5 bg-gray-800 rounded-lg text-sm text-violet-400 hover:bg-violet-500/20 transition-colors border border-gray-700"
                      >
          ...
          {activeTab === 'practice' && (
            <div className="animate-fade-in space-y-6">
              <h3 className="text-lg font-semibold mb-3 flex items-center gap-2">
                <Brain className="w-5 h-5 text-violet-400" /> Practice Questions
              </h3>
              {conceptDetail.practice_questions?.length > 0 ? (
                <div className="space-y-3">
                  {conceptDetail.practice_questions.map((q, i) => (
                    <div key={i} className="p-4 bg-gray-800/50 rounded-lg border border-gray-700/50 hover:border-violet-500/30 transition-colors">
                      <div className="flex items-start gap-3">
                        <span className="flex items-center justify-center w-7 h-7 rounded-full bg-violet-500/20 text-violet-400 text-sm font-bold flex-shrink-0">
                          {i + 1}
                        </span>
          ...
          {activeTab === 'videos' && (
            <div className="animate-fade-in">
              <VideoTutorials topicName={conceptDetail.youtube_keywords || conceptDetail.title + ' machine learning'} />
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <div className="animate-fade-in">
      {/* Hero Header */}
      <div className="glass-card p-6 mb-6 bg-gradient-to-br from-violet-500/10 to-fuchsia-500/5 border-violet-500/20">
        <div className="flex items-center gap-4 mb-4">
          ...
          <div>
            <h1 className="text-2xl font-bold">AI/ML Concepts</h1>
            <p className="text-gray-400">Master machine learning fundamentals to advanced AI</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
          <div className="p-3 bg-gray-800/50 rounded-lg text-center">
            <p className="text-2xl font-bold text-violet-400">{totalConcepts}</p>
            <p className="text-xs text-gray-400">Total Concepts</p>
          </div>
          <div className="p-3 bg-gray-800/50 rounded-lg text-center">
            <p className="text-2xl font-bold text-fuchsia-400">{overview.total_phases || 0}</p>
            <p className="text-xs text-gray-400">Phases</p>
          </div>
          <div className="p-3 bg-gray-800/50 rounded-lg text-center">
            <p className="text-2xl font-bold text-pink-400">{(overview.all_tags || []).length}</p>
      ...
      <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
        {Object.entries(overview.phases || {}).map(([num, phase]) => {
          const PhaseIcon = phaseIcons[Number(num)] || Cpu
        ...
        <div className="glass-card p-5 mb-6">
          <div className="flex items-start gap-3">
            <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${phaseColors[activePhase]} flex items-center justify-center flex-shrink-0`}>
              {(() => { const I = phaseIcons[activePhase] || Cpu; return <I className="w-5 h-5 text-white" /> })()}
            </div>
            <div>
              ...
              {overview.phases[activePhase].motivation_start && (
                <p className="text-xs text-violet-400/80 mt-2 italic">{overview.phases[activePhase].motivation_start}</p>
              )}
            </div>
          </div>
        </div>
      ...
      <div className="flex gap-3 mb-4 flex-wrap">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 bg-gray-800/60 border border-gray-700/50 rounded-lg text-sm text-gray-300 placeholder-gray-500 focus:outline-none focus:border-violet-500/50"
          />
        </div>
        <div className="flex gap-1 flex-wrap">
          <button
            onClick={() => setSelectedTag(null)}
            className={`text-xs px-3 py-1.5 rounded-full transition-colors ${!selectedTag ? 'bg-violet-500/20 text-violet-400 border border-violet-500/30' : 'bg-gray-800 text-gray-400 hover:text-gray-200'}`}
          >
            All
          </button>
          {(overview.all_tags || []).slice(0, 8).map(tag => (
            <button
              ...
              className={`text-xs px-3 py-1.5 rounded-full transition-colors ${selectedTag === tag ? 'bg-violet-500/20 text-violet-400 border border-violet-500/30' : 'bg-gray-800 text-gray-400 hover:text-gray-200'}`}
            >
              {tag}
            </button>
          ))}
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {filteredConcepts.map(concept => (
          <button
            ...
            className="glass-card p-5 text-left hover:border-violet-500/30 transition-all duration-200 group"
          >
            ...
            <div className="flex items-center gap-1 flex-wrap mb-3">
              {(concept.tags || []).map(tag => (
                <span key={tag} className="text-[10px] px-2 py-0.5 rounded-full bg-violet-500/10 text-violet-400/70">
                  {tag}
                </span>
              ))}
            </div>
      ...
      <form onSubmit={handleCustomSearch} className="flex gap-2">
        <div className="flex-1 relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 bg-gray-800/60 border border-gray-700 rounded-lg text-sm text-gray-200 placeholder-gray-500 focus:outline-none focus:border-violet-500/50"
          />
        </div>
        <button
          type="submit"
